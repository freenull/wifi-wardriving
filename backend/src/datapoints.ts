...
export async function submitDatapoint(
    ...
    if (req.ssid === undefined)
        return rest.errorResponse(
            rest.ErrorCode.MissingParameter,
            "Missing parameter 'ssid'",
        );
    if (req.bssid === undefined)
        return rest.errorResponse(
            rest.ErrorCode.MissingParameter,
            "Missing parameter 'bssid'",
        );
    if (req.auth_type === undefined)
        return rest.errorResponse(
            rest.ErrorCode.MissingParameter,
            "Missing parameter 'auth_type'",
        );
    if (!validateAuthType(req.auth_type))
        return rest.errorResponse(
            rest.ErrorCode.InvalidAuthType,
            `Invalid authentication type '${req.auth_type}'`,
        );

    let result = await db.query(
        "update datapoints set last_seen = now() where earth_distance(ll_to_earth(latitude, longitude), ll_to_earth($1, $2)) <= $3 and bssid = $4::macaddr returning datapoint_id;",
        [
            req.latitude,
            req.longitude,
    ...
    if (result.rows.length > 0) {
        return new rest.Response(200, {
            datapoint_id: result.rows[0].datapoint_id,
        });
    }

    result = await db.query(
        "insert into datapoints (latitude, longitude, ssid, bssid, auth_type, submitter, first_seen, last_seen) values ($1, $2, $3, $4, $5, $6, now(), now()) returning datapoint_id;",
        [
            req.latitude,
            req.longitude,
            req.ssid,
    ...
    if (result.rows.length < 1) {
        return rest.errorResponse(
            ...
            "Failed adding datapoint to database",
        );
    }
...
export async function retrieveDatapoint(
    db: pg.PoolClient,
    sessionData: session.SessionData,
    req: RetrieveByIdRequest,
): Promise<rest.Response> {
    ...
    let result = await db.query(
        "select latitude, longitude, ssid, bssid, auth_type, submitter, first_seen, last_seen from datapoints where datapoint_id = $1;",
        [req.datapoint_id],
    );

    if (result.rows.length < 1) {
        return rest.errorResponse(
            rest.ErrorCode.InvalidDatapointId,
...
export async function retrieveDatapoints(
    db: pg.PoolClient,
    sessionData: session.SessionData,
    req: RetrieveRequest,
): Promise<rest.Response> {
    ...
    let result = await db.query(
        "select datapoint_id, latitude, longitude, ssid, bssid, auth_type, submitter, first_seen, last_seen from datapoints where earth_distance(ll_to_earth(latitude, longitude), ll_to_earth($1, $2)) <= $3;",
        [req.latitude, req.longitude, req.radius ?? 10000],
    );
