...
export async function registerUser(
    ...
    if (sessionData.userId !== undefined) {
        return rest.errorResponse(
            rest.ErrorCode.NoLoginOnly,
            "Cannot use this endpoint while logged in",
        );
    }
    ...
    if (result.rows.length > 0) {
        return rest.errorResponse(
            rest.ErrorCode.UserRegistered,
            `User '${req.username}' already registered`,
        );
    }
    ...
    result = await db.query(
        "insert into users (username, key, salt) values ($1, $2, $3) returning user_id;",
        [req.username, derivedKey, salt],
    );
    if (result.rows.length < 1) {
        return rest.errorResponse(
            rest.ErrorCode.UserRegistered,
            `Failed registering username '${req.username}'`,
        );
    }
...
export async function loginUser(
    ...
    let result = await db.query(
        "select user_id, salt from users where username = $1::text;",
        [req.username],
    );
    if (result.rows.length < 1) {
        return rest.errorResponse(
            rest.ErrorCode.InvalidCredentials,
            `Invalid credentials for user '${req.username}'`,
        );
    }
    ...
    result = await db.query(
        "select user_id from users where user_id = $1 and key = $2;",
        [user_id, derivedKey],
    );
    if (result.rows.length < 1) {
        return rest.errorResponse(
...
export async function myUserData(
    db: pg.PoolClient,
    sessionData: session.SessionData,
): Promise<rest.Response> {
    if (sessionData.userId === undefined) {
    ...
    if (result.rows.length < 1) {
        return rest.errorResponse(
            ...
            "Failed getting user data from database",
        );
    }
