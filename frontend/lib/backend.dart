...
class Cluster {
    ...
    : datapoints = List<Datapoint>.from(data["datapoints"].map((datapointData) => Datapoint.fromDynamic(datapointData)))
...
class Backend {
    ...
    Future<bool> sendComment(ScaffoldMessengerState msgr, int datapointId, String comment) async {
        if (!await isLoggedIn(msgr)) {
            sendError(msgr, "Must be logged in to post comments.");
            return false;
        }

        final data = await doPost(msgr, "datapoints/$datapointId/comments", { "content": comment });
        if (data == null) return false;
    ...
    Future<List<Cluster>?> retrieveClusters(ScaffoldMessengerState msgr, LatLng center) async {
        final data = await doGet(msgr, "clusters/${center.latitude}/${center.longitude}");
        if (data == null) return null;
    ...
    Future<Datapoint?> retrieveDatapoint(ScaffoldMessengerState msgr, int datapointId) async {
    ...
    Future<bool> sendDatapoint(ScaffoldMessengerState msgr, Datapoint datapoint) async {
    ...
    Future<List<Achievement>?> retrieveOwnAchievements(ScaffoldMessengerState msgr) async {
        if (!await isLoggedIn(msgr)) {
        ...
        return List<Achievement>.from(data.map((achiev) => Achievement.fromDynamic(achiev)));
    }

    Future<List<LeaderboardEntry>?> retrieveLeaderboard(ScaffoldMessengerState msgr, int limit) async {
        final data = await doGet(msgr, "users/leaderboard?start=0&limit=$limit");
        if (data == null) return null;

        return List<LeaderboardEntry>.from(data.map((achiev) => LeaderboardEntry.fromDynamic(achiev)));
    }
}
